                ...
                <div className="space-y-2 mb-6">
                  {days.map((day) => (
                    <button key={day} onClick={() => setSelectedDay(day)}
                      className={`w-full text-left px-4 py-3 rounded-xl text-sm font-medium border transition-colors duration-200 ${selectedDay === day ? "bg-brand-700 text-white border-brand-700 shadow-sm" : "border-gray-200 text-gray-600 hover:border-brand-400"}`}>
                      {day}
                    </button>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {timeSlots.map((t) => (
                    <button key={t} onClick={() => setSelectedTime(t)}
                      className={`py-2 rounded-xl text-xs font-medium border transition-colors duration-200 ${selectedTime === t ? "bg-brand-700 text-white border-brand-700 shadow-sm" : "border-gray-200 text-gray-600 hover:border-brand-400"}`}>
                      {t}
                    </button>
                  ))}
                </div>
                ...
                <form onSubmit={handleBook} className="space-y-4">
                  {[
                    ...
                    <div key={label}>
                      ...
                      <input required type={type} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-400 transition-colors" placeholder={placeholder} />
                    </div>
                  ))}
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-1.5">What would you like to discuss?</label>
                    <textarea rows={3} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-400 transition-colors resize-none" placeholder="Project overview, challenges, goals..." />
                  </div>
                  <button type="submit" disabled={!selectedDay || !selectedTime}
                    className="w-full flex items-center justify-center gap-2 py-3 bg-brand-700 text-white font-bold rounded-xl shadow-sm disabled:opacity-40 hover:bg-brand-800 transition-colors">
